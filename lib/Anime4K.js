...
function onLoad() {
    ...
    const board = document.getElementById('velocity-canvas');
    const gl = board.getContext('webgl');

    player0.addEventListener('canplaythrough', function () {
        player0.play();
    }, true);
    player0.addEventListener('loadedmetadata', function () {
        let scale = parseFloat(2);

        scaler = new Scaler(gl);
        scaler.inputVideo(player0);
        scaler.resize(scale);
    }, true);
    player0.addEventListener('error', function () {
        alert("Can't load the video.");
    }, true);

    function render() {
        if (scaler) {
            scaler.bold = parseFloat(0);
            scaler.blur = parseFloat(0);
